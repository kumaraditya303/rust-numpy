...
#[doc(inline)]
#[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
pub use PyArrayObject_fields as PyArrayObject;
...
#[doc(inline)]
#[cfg(all(Py_LIMITED_API, Py_GIL_DISABLED))]
pub use _PyArray_DescrNumPy2 as PyArray_Descr_fields;

#[doc(inline)]
#[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
pub use PyArray_Descr_fields as PyArray_Descr;
...
#[doc(inline)]
#[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
pub(crate) use _PyArray_LegacyDescr_fields as _PyArray_LegacyDescr;
...
pub unsafe fn PyDataType_SET_ELSIZE<'py>(
    ...
    if is_numpy_2(py) {
        unsafe {
            (*_PyDataType_GET_ITEM_DATA(dtype).cast::<_PyArray_DescrNumPy2>()).elsize = size;
        }
    } else {
...
pub unsafe fn PyDataType_FLAGS<'py>(py: Python<'py>, dtype: *const PyArray_Descr) -> npy_uint64 {
    #[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
    if is_numpy_2(py) {
        unsafe { (*_PyDataType_GET_ITEM_DATA(dtype).cast::<_PyArray_DescrNumPy2>()).flags }
    } else {
        // Numpy 1 does not support abi3t, so direct field access is fine
...
#[doc(inline)]
#[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
pub use PyUFuncObject_fields as PyUFuncObject;
...
#[doc(inline)]
#[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
pub use PyArrayIterObject_fields as PyArrayIterObject;
...
#[doc(inline)]
#[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
pub use PyArrayMultiIterObject_fields as PyArrayMultiIterObject;
...
#[doc(inline)]
#[cfg(not(all(Py_LIMITED_API, Py_GIL_DISABLED)))]
pub use PyArrayNeighborhoodIterObject_fields as PyArrayNeighborhoodIterObject;
